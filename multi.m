clc;
clear all;
[s,Fs] = audioread('clean_speech.wav'); % audio
% [babble fbn] = audioread('babble_noise.wav');% noise
% y_nonstat = s + 0.3*babble(1:length(s));
% s=y_nonstat;
...
Y = ifft(Yk);
for j = 1 : M
    y(:,j) = overlap_add(Y(:,:,j), overlap_length, filter);
end
y([1 end], :) = 0;
y = y(1:length(s),:);
...
Shat = ifft(Skhat);
shat = overlap_add(Shat, overlap_length, filter);
shat([1 end]) = 0;
